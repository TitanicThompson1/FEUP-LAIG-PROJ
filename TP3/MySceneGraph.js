...
class MySceneGraph {
    ...
    parseXMLFile(rootElement) {
        ...
        if ((index = nodeNames.indexOf("model")) != -1) {
            ...
            if (index != indexCompontent) this.onXMLMinorError("tag <model> out of order");

            //Parse model block
            if ((error = this.parseModel(nodes[index])) != null) return error;
            indexCompontent++;
        }
    ...
    parseViews(viewsNode) {
        ...
        if (this.numCameras == 0) {
            ...
            this.views.set("default", new MyCamera(Math.PI / 6, 0.1, 500, vec3.fromValues(15, 15, 15), vec3.fromValues(0, 0, 0)));
            this.camerasID[this.numCameras++] = "default";
    ...
    parsePerspective(perspectiveNode, pID) {
        ...
        this.views.set(pID, new MyCamera(angle * DEGREE_TO_RAD, near, far, vec3.fromValues(xF, yF, zF), vec3.fromValues(xT, yT, zT)));
        this.camerasID[this.numCameras++] = pID;
    ...
    parseOrtho(orthoNode, oID) {
        ...
        this.camerasID[this.numCameras++] = oID;
        return null;
    }
    ...
    /**
     * Parses the <model> block.
     * @param {model block element} modelNode
     */
    parseModel(modelNode) {
        let children = modelNode.children;

        let modelNames = [];
        for (let i = 0; i < children.length; i++)
            modelNames.push(children[i].nodeName);

        let gameboardIndex = modelNames.indexOf("gameboard");
        let tileIndex = modelNames.indexOf("tile");
        let piecesIndex = modelNames.indexOf("pieces");
        let transfIndex = modelNames.indexOf("transformations");

        if (transfIndex != -1) {
            this.boardTransf = this.parseTransformations(children[transfIndex], "Model");
            console.log(this.boardTransf)
        }

        if (gameboardIndex != -1) {
            let material = this.reader.getString(children[gameboardIndex], 'material');
            let texture = this.reader.getString(children[gameboardIndex], 'texture');
            this.gameboardTheme = [];

            if (material != "null" && !this.materials.has(material)) {
                this.onXMLMinorError("There is no material with id " + material + ".");
                material = "null";
            }
            if ((texture != "null") && (texture != "clear") && (!this.textures.has(texture))) {
                this.onXMLMinorError("There is no texture with id " + texture + ". Clearing gameboard's texture.")
                texture = "null";
            }

            if (material == "null") this.gameboardTheme.push(material);
            else this.gameboardTheme.push(this.materials.get(material));

            if (texture == "null") this.gameboardTheme.push(texture);
            else this.gameboardTheme.push(this.textures.get(texture));


        } else {
            this.onXMLMinorError("There is no gameboard tag in model");
            return "Error";
        }

        if (tileIndex != -1) {
            let material = this.reader.getString(children[tileIndex], 'material');
            let texture = this.reader.getString(children[tileIndex], 'texture');

            this.tileTheme = [];

            if (material == "null") this.tileTheme.push("null");
            else if (this.materials.has(material)) this.tileTheme.push(this.materials.get(material));
            else {
                this.onXMLMinorError("There is no material with id " + material + ".");
                this.tileTheme.push("null")
            }

            if (texture == "null") this.tileTheme.push("null");
            else if (this.textures.has(texture)) this.tileTheme.push(this.textures.get(texture));
            else {
                this.onXMLMinorError("There is no texture with id " + texture + ". Clearing type3 piece's texture.");
                this.tileTheme.push("null");
            }
        } else {
            this.onXMLMinorError("There is no tile tag in model");
            return "Error";
        }

        if (piecesIndex != -1) {

            let grandChildren = children[piecesIndex].children;
            ...
            for (let i = 0; i < grandChildren.length; i++)
                piecesNames.push(grandChildren[i].nodeName);

            let type1Index = piecesNames.indexOf("type1");
            let type2Index = piecesNames.indexOf("type2");
            let type3Index = piecesNames.indexOf("type3");

            let mat1 = this.reader.getString(grandChildren[type1Index], 'material');
            let tex1 = this.reader.getString(grandChildren[type1Index], 'texture');

            if (mat1 == "null") pieceTheme.push("null");
            else if (this.materials.has(mat1)) pieceTheme.push(this.materials.get(mat1));
            else {
                this.onXMLMinorError("There is no material with id " + mat1 + ".");
                pieceTheme.push("null")
            }

            if (tex1 == "null") pieceTheme.push("null");
            else if (this.textures.has(tex1)) pieceTheme.push(this.textures.get(tex1));
            else {
                this.onXMLMinorError("There is no texture with id " + tex1 + ". Clearing type1 piece's texture.");
                pieceTheme.push("null");
            }

            this.piecesTheme.push(pieceTheme);
            pieceTheme = [];

            let mat2 = this.reader.getString(grandChildren[type2Index], 'material');
            let tex2 = this.reader.getString(grandChildren[type2Index], 'texture');

            if (mat2 == "null") pieceTheme.push("null");
            else if (this.materials.has(mat2)) pieceTheme.push(this.materials.get(mat2));
            else {
                this.onXMLMinorError("There is no material with id " + mat2 + ".");
                pieceTheme.push("null")
            }

            if (tex2 == "null") pieceTheme.push("null");
            else if (this.textures.has(tex2)) pieceTheme.push(this.textures.get(tex2));
            else {
                this.onXMLMinorError("There is no texture with id " + tex2 + ". Clearing type2 piece's texture.");
                pieceTheme.push("null");
            }
            ...
            let mat3 = this.reader.getString(grandChildren[type3Index], 'material');
            let tex3 = this.reader.getString(grandChildren[type3Index], 'texture');

            if (mat3 == "null") pieceTheme.push("null");
            else if (this.materials.has(mat3)) pieceTheme.push(this.materials.get(mat3));
            else {
                this.onXMLMinorError("There is no material with id " + mat3 + ".");
                pieceTheme.push("null")
            }

            if (tex3 == "null") pieceTheme.push("null");
            else if (this.textures.has(tex3)) pieceTheme.push(this.textures.get(tex3));
            else {
                this.onXMLMinorError("There is no texture with id " + tex3 + ". Clearing type3 piece's texture.");
                pieceTheme.push("null");
            }

            this.piecesTheme.push(pieceTheme);
        } else {
            this.onXMLMinorError("There is no pieces tag in model");
            return "Error";
        }

        this.log("Parsed model");
        return;
    }
    ...
    parseNodes(nodesNode) {
        ...
        for (var i = 0; i < children.length; i++) {
            ...
            if (transformationsIndex != -1) {
                nodeInfo.setTransformation(this.parseTransformations(grandChildren[transformationsIndex]), nodeID);
            } else {
    ...
    parseTransformations(transformationsNode, nodeID) {
